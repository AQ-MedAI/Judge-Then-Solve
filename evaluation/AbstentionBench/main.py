...
MODELS_WITHOUT_GPU = ["DummyModel", "GPT4oAPI", "o1API", "Gemini15ProAPI", "CustomOpenAIModel", "CustomOpenAIModelEnv"]

LLM_JUDGES_WITHOUT_GPU = [
    "ContainsAbstentionKeyword",
    "LLMJudgeGPT4o",
    "LLMJudgeGPT4oCustomAPI",
]
...
class AbstentionWorkflow:
    ...
    def raise_warning_two_models_within_one_job(self):
        ...
        # Safely access tensor_parallel_size, defaulting to None if not present
        # Use OmegaConf.select to avoid ConfigAttributeError if key doesn't exist
        try:
            inference_model_tensor_parallel = OmegaConf.select(
                self.config, "module.tensor_parallel_size", default=None
            )
            judge_model_tensor_parallel = OmegaConf.select(
                self.config, "abstention_detector.judge_model.tensor_parallel_size", default=None
            )
        except Exception:
            # If there's any error accessing these fields, skip the check
            return
        
        # If either model doesn't have tensor_parallel_size, skip the check
        if inference_model_tensor_parallel is None or judge_model_tensor_parallel is None:
            return
    ...
    def launch_evaluation_in_separate_job_with_submitit(self):
        ...
        # Save PYTHONPATH before clean_env() removes it
        project_root = os.path.dirname(os.path.abspath(__file__))
        saved_pythonpath = os.environ.get('PYTHONPATH', '')

        with clean_env():
            # Restore PYTHONPATH for pickle serialization
            if saved_pythonpath:
    ...
    def make_submitit_executor(self, logs_dir, slurm_launcher_configs: DictConfig):
        ...
        # Add project root to PYTHONPATH for submitted jobs
        import sys
        project_root = os.path.dirname(os.path.abspath(__file__))
        ...
        # Update slurm_launcher_configs with PYTHONPATH
        slurm_launcher_configs = dict(slurm_launcher_configs)
        if 'slurm_setup' not in slurm_launcher_configs:
            slurm_launcher_configs['slurm_setup'] = []
    ...
    def run_inference(self) -> RawResponses:
        ...
        use_prompt_method = self.config.get("use_prompt_method", False)
        inference_pipeline = InferencePipeline(
            model,
            datamodule,
            self.config.save_dir,
            batch_size=self.config.inference_batch_size,
            use_prompt_method=use_prompt_method,
        )
...
def save_config(config: DictConfig) -> None:
    # 如果启用了提示词方法，在 run_name 中添加标识
    if config.get("use_prompt_method", False):
        original_run_name = config.get("run_name", "")
        if not original_run_name.endswith("_ours_prompt"):
            config.run_name = f"{original_run_name}_ours_prompt"
            # 从已解析的 save_dir 和 logs_dir 中提取时间戳和路径信息
            # Hydra 已经解析了路径，我们需要替换 run_name 部分
            import re
            # 从 save_dir 中提取时间戳部分（格式：YYYY-MM-DD_HH-MM-SS）
            save_dir_str = str(config.save_dir)
            # 匹配时间戳模式
            timestamp_match = re.search(r'/(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})/?$', save_dir_str)
            if timestamp_match:
                timestamp = timestamp_match.group(1)
                # 替换 run_name 部分
                # 格式: .../results[/sweep_folder]/OLD_RUN_NAME/timestamp
                # 需要替换为: .../results[/sweep_folder]/NEW_RUN_NAME/timestamp
                old_run_name_pattern = re.escape(original_run_name)
                new_save_dir = re.sub(
                    f'/{old_run_name_pattern}/({re.escape(timestamp)})',
                    f'/{config.run_name}/{timestamp}',
                    save_dir_str
                )
                new_logs_dir = re.sub(
                    f'/{old_run_name_pattern}/({re.escape(timestamp)})',
                ...
                # 如果无法提取时间戳，使用原始方法
                logger.warning("无法从路径中提取时间戳，使用默认方法更新路径")
                sweep_folder = config.get("sweep_folder", "")
                common_dir = config.get("common_dir", "")
                from datetime import datetime
                now_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                if sweep_folder:
                    config.save_dir = f"{common_dir}/results/{sweep_folder}/{config.run_name}/{now_str}"
                    config.logs_dir = f"{common_dir}/logs/{sweep_folder}/{config.run_name}/{now_str}"
                else:
                    config.save_dir = f"{common_dir}/results/{config.run_name}/{now_str}"
                    config.logs_dir = f"{common_dir}/logs/{config.run_name}/{now_str}"
