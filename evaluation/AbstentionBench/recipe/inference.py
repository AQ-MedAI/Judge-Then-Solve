...
class InferencePipeline:
    ...
    def __init__(
        ...
        save_dir: str,
        batch_size: int = 1,
        use_prompt_method: bool = False,
    ):
        self.model = model
        self.dataset = dataset
        self.save_dir = save_dir
        self.batch_size = batch_size
        self.use_prompt_method = use_prompt_method
        # 定义提示词方法的内容
        self.prompt_method_suffix = (
            "[Question] Before reasoning deeply, "
            "check whether all necessary information is available. "
            "If any key data is missing or ambiguous, explicitly state that first; "
            "otherwise, answer it with the minimum number of tokens required."
        )
    ...
    def run(self, indices_subset=None) -> RawResponses:
        ...
        for prompt_batch in batched_dataset:
            ...
            # 如果启用了提示词方法，在问题后添加提示词
            if self.use_prompt_method:
                questions = [f"{q} {self.prompt_method_suffix}" for q in questions]
