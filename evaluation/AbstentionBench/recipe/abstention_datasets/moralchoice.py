...
class MoralChoiceDataset(AbstentionDataset):
    ...
    def __init__(
        ...
        data_dir="data/moralchoice",
    ):
        # 从本地加载数据
        all_templates = datasets.load_from_disk(f"{data_dir}_templates")["train"]
        ...
        # 从本地加载数据
        self.dataset = datasets.load_from_disk(f"{data_dir}_scenarios")["train"]
