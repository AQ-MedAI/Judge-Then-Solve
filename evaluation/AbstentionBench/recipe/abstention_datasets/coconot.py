...
class CoCoNotDataset(AbstentionDataset):
    ...
    def __init__(self, max_num_samples=None, data_dir="data/coconot"):
        super().__init__()

        # 从本地加载数据
        coconot_original = datasets.load_from_disk(f"{data_dir}_original")
        coconot_contrast = datasets.load_from_disk(f"{data_dir}_contrast")
