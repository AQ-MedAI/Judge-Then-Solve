...
async def eval_rollout_single_dataset(
    ...
    # For group_rm, compute rewards for all samples at once
    if args.group_rm:
        rewards = await batched_async_rm(args, data)
        for sample, reward in zip(data, rewards, strict=False):
            sample.reward = reward

    reward_key = args.eval_reward_key or args.reward_key
    # Extract rewards properly handling both dict and float types
    rewards = []
    for sample in data:
        if sample.reward is None:
            rewards.append(None)
        elif reward_key and isinstance(sample.reward, dict):
            rewards.append(sample.reward.get(reward_key, None))
        else:
            # If no reward_key or reward is not a dict (e.g., float), use reward directly
