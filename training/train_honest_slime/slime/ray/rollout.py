...
def _log_eval_rollout_data(rollout_id, args, data, extra_metrics: dict[str, Any] | None = None):
    ...
    for key in data.keys():
        ...
        if rewards:
            ...
            log_dict[f"eval/{key}"] = 0.0
        if (samples := data[key].get("samples")) is not None:
            log_dict |= dict_add_prefix(compute_metrics_from_samples(args, samples), f"eval/{key}/")
            
            # Compute reward by label (for honesty training: insufficient vs well_defined)
            # Group samples by question_label from metadata
            samples_by_label = group_by(samples, lambda s: s.metadata.get("question_label", "unknown") if isinstance(s.metadata, dict) else "unknown")
            
            for label, label_samples in samples_by_label.items():
                if label != "unknown":  # Only log known labels
                    label_rewards = [s.get_reward_value(args) for s in label_samples if s.get_reward_value(args) is not None]
                    if len(label_rewards) > 0:
                        log_dict[f"eval/{key}/reward_by_label/{label}"] = sum(label_rewards) / len(label_rewards)
                        log_dict[f"eval/{key}/count_by_label/{label}"] = len(label_rewards)
                    # Raw r_task: purely based on eval_result, no format/consistency gating
                    r_task_raw = [s.metadata["r_task_raw"] for s in label_samples if isinstance(s.metadata, dict) and "r_task_raw" in s.metadata]
                    if len(r_task_raw) > 0:
                        log_dict[f"eval/{key}/r_task_raw_by_label/{label}"] = sum(r_task_raw) / len(r_task_raw)
...
def _compute_zero_std_metrics(args, all_samples: list[Sample]):
    ...
    interesting_rewards = [str(round(g[0].get_reward_value(args), 1)) if g[0].get_reward_value(args) is not None else "0.0" for g in interesting_sample_groups]
