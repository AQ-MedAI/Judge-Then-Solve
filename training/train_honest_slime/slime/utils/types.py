...
class Sample:
    ...
    def get_reward_value(self, args) -> float:
        if not args.reward_key:
            return self.reward
        # If reward_key is set, check if reward is a dict
        if isinstance(self.reward, dict):
            return self.reward.get(args.reward_key, None)
        else:
            # If reward is not a dict (e.g., float), return it directly
            # This handles cases where reward function returns float but reward_key is set
            return self.reward
